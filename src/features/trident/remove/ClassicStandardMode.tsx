    ...
    <ListPanel.Item
      left={
        <div className="flex flex-row gap-1.5 lg:gap-3 items-center">
          ...
          >
            {receiveNative
              ? i18n._(t`Receive WETH instead`)
              : i18n._(t`Receive ETH instead`)}
          </Typography>
        </div>
      }
      right={
        <ListPanel.Item.Right>
          ≈${usdcValue ? usdcValue?.toFixed(2) : "0.00"}
        </ListPanel.Item.Right>
      }
      key={0}
    />
...
const ClassicStandardMode: FC = () => {
  ...
  const dispatch = useAppDispatch();
  const { attemptingTxn, bentoPermit, outputToWallet, percentageAmount } =
    useAppSelector(selectTridentRemove);
  ...
  return (
    <SumUSDCValues amounts={liquidityValue}>
      {({ amount }) => {
        const selectedLiquidityValueInUsdc = amount?.multiply(
          new Percent(percentageAmount, "100")
        );
        return (
            ...
            <div className="flex flex-col gap-3 lg:gap-6">
              <div className="flex items-center justify-between gap-10">
                <Typography
                  variant="h3"
                  weight={700}
                  className="mb-2 text-high-emphesis lg:mb-0"
                >
                  Amount to Remove:
                </Typography>
              ...
              <ListPanel
                header={
                  <ListPanel.Header
                    ...
                    subValue={`${
                      poolBalance ? poolBalance.toSignificant(6) : "0.0000"
                    } SLP`}
                  />
                }
                items={[
                  liquidityValue.map((el, index) => (
                    <CurrencyAmountItemWithEthSelector
                      amount={el}
                      key={index}
                    />
                  )),
                ...
                footer={
                  <div className="flex items-center justify-between gap-3 px-4 py-6">
                    <PercentInput
                      value={percentageAmount}
                      onUserInput={(val) =>
                        dispatch(setRemovePercentageAmount(val))
                      }
                      placeholder="0%"
                      className="bg-transparent text-3xl leading-7 tracking-[-0.01em] flex-grow font-bold text-white"
                    />
                    <Typography
                      variant="lg"
              ...
              >
                {({ approved, loading }) => {
                  const disabled =
                    !!error || !approved || loading || attemptingTxn;
                  ...
                  return (
                    <Button
                      ...
                      loading={loading}
                      color={approved ? "btn_primary" : "btn_primary"}
                      disabled={disabled}
                      onClick={() => dispatch(setRemoveShowReview(true))}
                      className="!h-10 rounded-[0.350rem] !text-white"
                    >
                      <Typography
                        variant="sm"
                        weight={700}
                        className={!error ? "text-white" : "text-low-emphasis"}
                      >
                        {buttonText}
                      </Typography>
                    </Button>
            ...
            <div className="flex flex-col block gap-4 lg:hidden">
              <div className="flex justify-between gap-3">
                ...
                <AssetInput.WalletSwitch
                  ...
                  onChange={() =>
                    dispatch(setRemoveOutputToWallet(!outputToWallet))
                  }
              ...
              <div className="flex flex-col gap-4">
                <ListPanel
                  ...
                  footer={
                    <div className="flex justify-between px-4 py-3.5 bg-filed">
