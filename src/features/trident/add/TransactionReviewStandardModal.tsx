...
const TransactionReviewStandardModal: FC = () => {
  ...
  const { attemptingTxn, showReview, txHash, spendFromWallet, bentoPermit } =
    useAppSelector(selectTridentAdd);
  ...
  const parsedAmounts = [
    _parsedAmounts[0] ||
      (currencies?.[0]
        ? CurrencyAmount.fromRawAmount(currencies[0], "0")
        : undefined),
    _parsedAmounts[1] ||
      (currencies?.[1]
        ? CurrencyAmount.fromRawAmount(currencies[1], "0")
        : undefined),
  ];
      ...
      {!txHash ? (
        <div className="flex flex-col gap-4">
          ...
          <Typography variant="sm">
            {i18n._(
              t`Output is estimated. If the price changes by more than 0.5% your transaction will revert.`
            )}
          </Typography>
          ...
          <div className="flex justify-between px-2 py-1">
            ...
            >
              {poolShareBefore?.greaterThan(0)
                ? poolShareBefore?.toSignificant(6)
                : "0.000"}
        ...
        <DepositSubmittedModalContent
          txHash={txHash}
          onDismiss={() => dispatch(setAddTxHash(undefined))}
        />
      )}
    </HeadlessUIModal.Controlled>
  );
