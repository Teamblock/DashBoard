...
const InariButton: FC<InariButtonProps> = ({ children, ...rest }) => {
  ...
  if (approveCallback && approveCallback[0] === ApprovalState.NOT_APPROVED)
    return (
      <>
        <Button {...rest} color="btn_primary" onClick={approveCallback[1]}>
          {/*@ts-ignore TYPE NEEDS FIXING*/}
          {i18n._(
  ...
  if (
    ...
    return (
      <>
        <Button {...rest} color="btn_primary" onClick={handleGetPermit}>
  ...
  return (
    <Button
      {...rest}
      disabled={pending}
      color={pending ? "gray" : "btn_primary"}
      onClick={onExecute}
    >
      {children}
    </Button>
