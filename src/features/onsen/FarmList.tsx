...
import {
  ...
  setOnsenModalState,
  setOnsenModalView,
} from "app/features/onsen/onsenSlice";
import {
...
const SortIcon: FC<{
  ...
  if (direction === "ascending")
    return <ChevronUpIcon width={12} height={12} />;
  if (direction === "descending")
    return <ChevronDownIcon width={12} height={12} />;
  return <></>;
};
  ...
  useEffect(() => {
    if (cardId) {
      ...
      if (selected) {
        ...
        dispatch(
          setOnsenModalState({
            view: positionIds.includes(selected.id)
              ? OnsenModalView.Position
              : OnsenModalView.Liquidity,
            open: true,
          })
        );
      }
    ...
    <>
      <div className="w-full hide-scrollbar overflow-auto">
        <div className="flex justify-between border border-1 border-gray-500 min-w-[768px] rounded-[0.350rem] !w-full overflow-auto">
          ...
          <div
            className={classNames(
              "flex gap-1 maxLg:pl-8 items-center cursor-pointer justify-end",
          ...
          <div
            className={classNames(
              "flex gap-1 items-center cursor-pointer justify-end",
          ...
          >
            ...
            <SortIcon
              id={sortConfig.key}
              direction={sortConfig.direction}
              active={sortConfig.key === "roiPerYear"}
            />
          </div>
        </div>
            ...
            {items.slice(0, numDisplayed).map((farm, index) => (
              <FarmListItem
                ...
                onClick={() => {
                  ...
                  dispatch(
                    setOnsenModalState({
                      view: positionIds.includes(farm.id)
                        ? OnsenModalView.Position
                        : OnsenModalView.Liquidity,
                      open: true,
                    })
