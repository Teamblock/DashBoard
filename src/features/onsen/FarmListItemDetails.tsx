...
const COLUMN_CONTAINER = "flex flex-col flex-grow gap-4 w-full";
      ...
      <div className={classNames("")}>
        ...
        <div className={classNames(COLUMN_CONTAINER, content ? "hidden" : "")}>
          <HeadlessUiModal.Header
            header={
              <div className="flex gap-0.5 items-center">
                ...
                <QuestionHelper
                  className="!max-w-[17rem] p-1 font-normal bg-popupBg border border-Gray rounded-md text-sm overflow-auto hide-scrollbar"
          ...
          <ToggleButtonGroup
            size="sm"
            value={view}
            onChange={(view: OnsenModalView) =>
              dispatch(setOnsenModalView(view))
            }
            variant="filled"
          >
            ...
            <ToggleButtonGroup.Button value={OnsenModalView.Staking}>
              {i18n._(t`Farm`)}
            </ToggleButtonGroup.Button>
            <ToggleButtonGroup.Button
          ...
          <div
            className={classNames(
              COLUMN_CONTAINER,
              view === OnsenModalView.Position ? "block" : "hidden"
            )}
          >
          ...
          <div
            className={classNames(
              COLUMN_CONTAINER,
              view === OnsenModalView.Liquidity ? "block" : "hidden"
            )}
          >
            {farm.pair.type === PairType.KASHI ? (
          ...
          <div
            className={classNames(
              COLUMN_CONTAINER,
              view === OnsenModalView.Staking ? "block" : "hidden"
            )}
          >
            <ManageBar farm={farm} />
          </div>
...
export const useFarmListItemDetailsModal = () => {
  ...
  if (!context) {
    throw new Error(
      "Hook can only be used inside Farm List Item Details Context"
    );
  }
