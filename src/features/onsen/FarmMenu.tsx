...
const MenuLink: FC<{ href?: string; label: string; onClick?(): void }> = ({
  href,
  label,
  onClick,
}) => {
...
const OnsenFilter = () => {
  ...
  const [selected, setSelected] = useState<FarmFilter>(
    filters[filter] || FarmFilter.All
  );

  const items = useMemo(() => {
    const map: Record<string, ReactNode> = {
      [FarmFilter?.All]: (
        <MenuLink href={"/farm"} label={i18n._(t`All Farms`)} />
      ),
      [FarmFilter?.Portfolio]: account ? (
        <MenuLink
          href={"/farm?filter=portfolio"}
          label={i18n._(t`Your Farms`)}
        />
      ) : (
    ...
    return Object.entries(map).reduce<Record<string, ReactNode>>(
      (acc, [k, v]) => {
    ...
    <div className="flex gap-2 items-center w-full md:w-[180px]">
      <Menu as="div" className="relative inline-block w-full text-left">
        ...
        >
          <Menu.Items
            static
            className="absolute w-full z-10 mt-0 border divide-y rounded shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none border-Gray bg-field divide-Gray overflow-hidden border-top-0"
          >
