...
const PoolWithdraw = ({ currencyA, currencyB, header }) => {
  ...
  useETH &&
    currencyA &&
    currencyEquals(currencyA, WNATIVE[chainId]) &&
    (currencyA = NATIVE[chainId]);
  // @ts-ignore TYPE NEEDS FIXING
  useETH &&
    currencyB &&
    currencyEquals(currencyB, WNATIVE[chainId]) &&
    (currencyB = NATIVE[chainId]);
      ...
      await routerContract[methodName](...args, {
        ...
        .then((response: TransactionResponse) => {
          ...
          addTransaction(response, {
            summary: t`Remove ${parsedAmounts[Field.CURRENCY_A]?.toSignificant(
              3
  ...
  return (
    <>
      <HeadlessUiModal.BorderedContent className="flex flex-col gap-4">
        ...
        <div className="flex justify-between mx-2 mt-2 items-center">
          ...
          {(oneCurrencyIsETH || oneCurrencyIsWETH) && (
            <Button
              variant="outlined"
              className="!rounded-[0.350rem] !font-normal text-white !h-8"
              color="green"
        ...
        <ListPanel
          items={Object.values(amountsMinCurrencyAmount).map(
            (parsedInputAmount, index) => (
      ...
      {!userLiquidity?.equalTo(ZERO) && (
        <Typography variant="xs" className="italic text-center hidden">
          {i18n._(
            t`If your ELP is staked, you cannot remove your liquidity. You must unstake first.`
          )}
        </Typography>
      )}
      {!account ? (
