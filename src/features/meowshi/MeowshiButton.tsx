...
const MeowshiButton: FC<MeowshiButtonProps> = ({ meowshiState }) => {
  ...
  if (!account)
    return (
      <Button onClick={approve} color="btn_primary" className='h-10 rounded-[0.350rem]' disabled={true}>
  ...
  if (approvalState === ApprovalState.PENDING)
    return (
      <Button color="btn_primary" className='h-10 rounded-[0.350rem]' disabled={true}>
  ...
  if (approvalState === ApprovalState.APPROVED)
    return (
      <>
        ...
        />
        <Button onClick={handleSubmit} disabled={!!buttonDisabledText} color="btn_primary" className='h-10 rounded-[0.350rem]'>
