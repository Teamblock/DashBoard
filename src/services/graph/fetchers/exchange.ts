...
export const EXCHANGE = {
  ...
  [ChainId.MOONBEAM]: "energyfidevops/moonbeam-exchange",
  [ChainId.MATIC_TESTNET]: "abhishekwin/twin-exchange",
  [ChainId.MOONBEAM_TESTNET]: "chandu-thoughtwin/exchange",
...
export const getPairDayData = async (chainId = ChainId.MATIC_TESTNET, variables) => {
  ...
  const { pairDayDatas } = await exchange(
    chainId,
    pairDayDatasQuery,
    variables
  );
...
// @ts-ignore TYPE NEEDS FIXING
export const getTokenSubset = async (chainId = ChainId.MATIC_TESTNET, variables) => {
...
// @ts-ignore TYPE NEEDS FIXING
export const getTokens = async (chainId = ChainId.MATIC_TESTNET, variables) => {
  // console.log('getTokens')
...
export const getTokenDayData = async (
  ...
  // console.log('getTokens')
  const { tokenDayDatas } = await exchange(
    chainId,
    tokenDayDatasQuery,
    variables
  );
...
// @ts-ignore TYPE NEEDS FIXING
export const getTokenPrices = async (chainId = ChainId.MATIC_TESTNET, variables) => {
...
export const getDevPrice = async (variables = {}) => {
  return getTokenPrice(ChainId.MOONBEAM_TESTNET, tokenPriceQuery, {
    id: "0x372d0695E75563D9180F8CE31c9924D7e8aaac47",
    ...variables,
  });
};

export const getMaticTestPrice = async (variables = {}) => {
...
export const getTokenPairs = async (
  ...
  const { pairs0, pairs1 } = await exchange(
    chainId,
    tokenPairsQuery,
    variables
  );
  return pairs0 || pairs1
    ? [...(pairs0 ? pairs0 : []), ...(pairs1 ? pairs1 : [])]
    : undefined;
};
