...
export const BLOCKS = {
  ...
  [ChainId.MOONBEAM]: 'energyfidevops/blocks',
  [ChainId.MOONBEAM_TESTNET]: 'chandu-thoughtwin/blocks',
  [ChainId.MATIC_TESTNET]: 'abhishekwin/twin-block',


}

// @ts-ignore TYPE NEEDS FIXING
const fetcher = async (chainId = ChainId.MATIC_TESTNET, query, variables = undefined) => {
  // @ts-ignore TYPE NEEDS FIXING
  return request(`${GRAPH_HOST[chainId]}/subgraphs/name/${BLOCKS[chainId]}`, query, variables)
}

// @ts-ignore TYPE NEEDS FIXING
export const getBlock = async (chainId = ChainId.MATIC_TESTNET, variables) => {
...
// @ts-ignore TYPE NEEDS FIXING
export const getBlocks = async (chainId = ChainId.MATIC_TESTNET, variables) => {
  const { blocks } = await fetcher(chainId, blocksQuery, variables)
  return blocks
}

// @ts-ignore TYPE NEEDS FIXING
export const getMassBlocks = async (chainId = ChainId.MATIC_TESTNET, timestamps) => {
...
export const getAverageBlockTime = async (chainId = ChainId.MATIC_TESTNET) => {
