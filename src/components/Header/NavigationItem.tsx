...
import {
  ...
  MenuItemLeaf,
  MenuItemNode,
} from "../../components/Header/useMenu";
// @ts-ignore
...
} from "app/hooks/useDesktopMediaQuery";
...
export const NavigationItem: FC<NavigationItem> = ({ node, onClose }) => {
  ...
  if (node && node.hasOwnProperty("link")) {
    ...
    return (
      <Typography
        onClick={() => {
          router.push(link), onClose && onClose();
        }}
        weight={700}
        variant="sm"
        className={classNames(
          router.asPath === link ? "text-white" : "text-white",
          "hover:text-white text-white font-bold py-6 px-2 rounded flex gap-3 maxSm:font-semibold"
        )}
      >
        {!isDesktop && node.icon}
        {node.title}
        ...
        >
          <Popover.Button ref={buttonRef}>
            <Typography
              ...
              className={classNames(
                open ? "text-white" : "",
                "hover:text-white text-white font-bold py-5 px-2 rounded flex gap-3 maxSm:font-semibold"
              )}
            >
              {!isDesktop && node.icon}
              {node.title}
              <ChevronDownIcon
                strokeWidth={3}
                width={12}
                className=" maxSm:absolute maxSm:right-4 maxSm:mt-1"
              />
            </Typography>
          </Popover.Button>
          {node.hasOwnProperty("items") && (
            ...
            >
              <Popover.Panel className="z-10 w-full absolute w-40 translate-y-[-5px] translate-x-[-8px]">
                <div
                  className={classNames(
                    "shadow-md shadow-drop_shadow2 bg-field border hover:text-white/80 border-Gray rounded-[0.350rem] overflow-hidden min-w-[8rem]",
                    !touchDevice
                ...
                >
                  {(node as MenuItemNode).items.map((leaf) => (
                    <Typography
                      ...
                      className=" p-2 flex items-center gap-2 hover:cursor-pointer text-white hover:text-white/80 m-0 rounded"
                    >
                      {leaf.icon}
                      {leaf.title}
                    </Typography>
