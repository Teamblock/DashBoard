...
export default function Table<T>({
  ...
  const [isHidden, setHidden] = useState(
    columnsHideable.length === 0 ? false : true
  );
  ...
  const toggleHide = (e) => {
    // @ts-ignore TYPE NEEDS FIXING
    const list = allColumns.filter((column) =>
      columnsHideable.find((c) => c === column.id)
    );
    // @ts-ignore TYPE NEEDS FIXING
    ...
    <>
              ...
              <tr key="tr" {...headerGroup.getHeaderGroupProps()}>
                ...
                {headerGroup.headers.map((column, i, columns) => (
                  ...
                  >
                    ...
                    >
                      <div className="flex flex-row text-sm text-black py-4 !font-normal">
                        <div
                          className={classNames(
                            //@ts-ignore TYPE NEEDS FIXING
                            i !== 0 &&
                        ...
                        >
                          ...
                          <span
                            className={classNames(
                              "flex items-center",
                              column.isSorted ? "block" : "hidden"
                            )}
                          >
                            <div
                              className={classNames(
                                "pl-1 fill-current text-white/80 !font-normal",
                                // @ts-ignore TYPE NEEDS FIXING
                          ...
                          onClick={
                            link
                              ? () =>
                                  router.push(
                                    link.href +
                                      getProperty(cell.row.original, link.id)
                                  )
                              : () => {}
                          }
                        >
                          <div
                            className={classNames(
                              ...
                              link && "cursor-pointer",
                              "h-20 text-white bg-[#4F4F4F] flex items-center"
                            )}
                          >
                            <div
                              className={classNames(
      ...
      {data?.length > 10 && (
        <div className="flex justify-between w-full my-4">
          <div className="flex text-sm font-normal text-secondary">
