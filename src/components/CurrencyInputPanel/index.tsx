...
export default function CurrencyInputPanel({
    ...
    return (
      ...
      >
        <div
          className={
            currencySelectCss ||
            "border !flex rounded-md border-gray-600 h-14 relative"
          }
        >
          <div className="flex">
            {pair ? (
                ...
                <div className="flex items-center">
                  ...
                  {!disableCurrencySelect && currency && (
                    <ChevronDownIcon
                      width={16}
                      height={16}
                      className="ml-2 stroke-current  absolute right-4"
                    />
                  )}
                </div>
              </div>
  ...
  const InputField: JSX.Element = React.useMemo(() => {
    return (
      <div>
        {!hideInput && (
          <div className={classNames("flex items-center w-full space-x-3")}>
            <>
              <Input.Numeric
  ...
  return (
    <div
      ...
      className={classNames(
        hideInput
          ? "p-4"
          : "px-6 maxMd:px-4 pt-6 pb-10 rounded-[0.350rem] bg-ternary",
        CurrencyInputClass
      )}
    >
      <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row relative">
        <div className={classNames("w-full")}>
          {label && (
            <div className="text-xs font-medium text-secondary whitespace-nowrap mb-1.5">
              {label}
            </div>
          )}
          <div
            className={classNames(
              "grid grid-cols-2 gap-6 maxMd:grid-cols-1 maxMd:gap-[0.5rem] items-center w-full"
            )}
          >
            <CurrencySelect />
