...
export default function Token() {
  ...
  const nativePrice1d = useNativePrice({
    chainId,
  ...
  return (
    <AnalyticsContainer>
      <Container maxWidth="6xl" className="mx-auto w-full h-full p-3">
        ...
        <div className="flex flex-col gap-6 juastify-center mb-5"></div>
                  ...
                  <div className="rounded-3xl text-sm bg-[#414A6C] maxMd:w-full maxMd:!max-w-[10rem] py-1 px-2 max-h-[2rem] flex items-center gap-2">
                    ...
                    >
                      {isCopied ? (
                        ...
                        <DuplicateIcon height={16} className="scale-x-[-1]" />
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-8 justify-around items-center w-full">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="text-lg font-medium text-white">
                      {formatNumber(
                        price * (totalSupply / 10 ** token?.decimals) ?? 0,
                        true,
                        false
                      )}
                    </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-row justify-between flex-grow space-x-4 overflow-x-auto hide-scrollbar">
            <InfoCard
              text="Liquidity (24H)"
              number={liquidityUSD}
              percent={liquidityUSDChange}
            />
            <InfoCard
              text="Volume (24H)"
              number={volumeUSD1d}
              percent={volumeUSD1dChange}
            />
            <InfoCard
              text="Fees (24H)"
              number={volumeUSD1d * 0.003}
              percent={volumeUSD1dChange}
            />
          </div>
            ...
            <table className="w-full table-fixed">
              ...
              <tbody>
                <tr>
                  ...
                  <td>{token?.symbol}</td>
                  <td>
                    <div className="w-11/12 overflow-hidden cursor-pointer text-purple overflow-ellipsis whitespace-nowrap">
                      {id}
                    </div>
                  </td>
                  <td>
          ...
          {/*@ts-ignore TYPE NEEDS FIXING*/}
          <LegacyTransactions
            pairs={tokenPairs ? tokenPairs.map((pair) => pair.id) : []}
          />
        </div>
      </Container>
    </AnalyticsContainer>
