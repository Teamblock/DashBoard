...
export default function xEnergyfi() {
  ...
  const dayData = useDayData({ chainId: ChainId.MATIC_TESTNET });
  ...
  const xEnergyFI1d = useTokens({
    ...
    variables: {
      block: block1d,
      where: { id: XTWT_CALL.address.toLowerCase() },
    },
  })?.[0];
  ...
  const energyFIDayData = useTokenDayData({
    chainId: ChainId.MATIC_TESTNET,
    variables: {
      where: { token: SUSHI_ADDRESS[ChainId.MATIC_TESTNET].toLowerCase() },
    },
  });
      ...
      barHistory && dayData && energyFIDayData && bar
        ? // @ts-ignore TYPE NEEDS FIXING
          barHistory.map((barDay) => {
            ...
            const totalEnergyFIStakedUSD =
              barDay.xEnergyFISupply * barDay.ratio * energyFIDay?.priceUSD;

            const APR =
              totalEnergyFIStakedUSD !== 0
                ? ((exchangeDay.volumeUSD * 0.0005 * 365) /
                    totalEnergyFIStakedUSD) *
                  100
                : 0;
  ...
  const APY1d = aprToApy(
    (((exchange?.volumeUSD - exchange1d?.volumeUSD) * 0.0005 * 365.25) /
      (bar?.totalSupply * xEnergyFIPrice)) *
      100 ?? 0
  );
  // @ts-ignore TYPE NEEDS FIXING
  const APY1w = aprToApy(
    data.slice(-7).reduce((acc, day) => (acc += day.APY), 0) / 7
  );
    ...
    <AnalyticsContainer>
      <Container maxWidth="6xl" className="mx-auto w-full h-full mb-8">
        <div className="py-10">
          <BackIcon
            onClick={() => router.back()}
            className="cursor-pointer mt-5 mb-8"
          />
            ...
            <div className="flex justify-end space-x-12">
              <div className="flex flex-col">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <ColoredNumber
                    number={(xEnergyFIPrice / xEnergyFIPrice1d) * 100 - 100}
                    percent={true}
                  />
                </div>
              </div>
              <div className="flex flex-col">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <ColoredNumber
                    number={
                      (xEnergyFIMarketcap / xEnergyFIMarketcap1d) * 100 - 100
                    }
                    percent={true}
                  />
                </div>
        ...
        <div className="pt-4 space-y-5">
          <div className="flex flex-col md:flex-row w-full gap-3">
            <InfoCard
              text="APY (Last 24 Hours)"
              number={formatPercent(APY1d)}
            />
