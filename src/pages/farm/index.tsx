...
export default function Farm(): JSX.Element {
  ...
  return (
    <div className="px-2 pt-10 pb-10 md:pb-20 w-full">
      <Container
        maxWidth="5xl"
        className="bg-black/30 mx-auto p-3 md:p-8 rounded-[0.350rem]"
      >
        <TridentHeader className="flex flex-col justify-center items-center">
          <div className="flex flex-col justify-center items-center">
            <Typography variant="lg" className="text-white" weight={700}>
              {i18n._(t`Farming Menu`)}
            </Typography>
            <Typography variant="sm" weight={400} className="text-white mb-4">
              {i18n._(
          ...
          <div className="flex flex-col w-full gap-6 mt-3 md:mt-5">
            <div className="flex flex-col sm:flex-row justify-between gap-4 items-center">
              <Search
                search={search}
                term={term}
                searchBarCss="border-Gray !bg-ternary"
              />
              <OnsenFilter />
            </div>
            ...
            {chainId && chainId === ChainId.CELO && (
              <Typography
                variant="xs"
                weight={700}
                className="text-secondary italic text-center"
              >
                {i18n._(
                  t`Users can now bridge back to Celo using a new version of Optics.`
                )}{" "}
                <ExternalLink
                  color="primary"
