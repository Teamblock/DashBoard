                ...
                href={
                  poolWithState?.state === PoolState.EXISTS
                    ? {
                        ...
                        query: {
                          tokens: [
                            isWrappedReturnNativeSymbol(
                              chainId,
                              poolWithState.pool.token0.address
                            ),
                            isWrappedReturnNativeSymbol(
                              chainId,
                              poolWithState.pool.token1.address
                            ),
                          ],
          ...
          <div>
            <div className="flex flex-col gap-1">
              ...
              <Typography variant="sm">
                {i18n._(
                  t`Deposit tokens in equal amounts or deposit either one of the two tokens or in any ratio.`
                )}
              </Typography>
              {poolWithState &&
                [PoolState.NOT_EXISTS, PoolState.INVALID].includes(
                  poolWithState.state
                ) && (
                  <Alert
                    ...
                    showIcon
                    message={i18n._(
                      t`A Pool could not be found for provided currencies`
                    )}
                  />
                )}
            </div>
