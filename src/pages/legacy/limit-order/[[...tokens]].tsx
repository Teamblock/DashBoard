...
const LimitOrder = () => {
  ...
  const { typedField, typedValue, fromBentoBalance, recipient } =
    useLimitOrderState();
  ...
  const pairs = useMemo(
    ...
    () =>
      (limitOrderPairList.pairs[chainId || 1] || []).map(([token0, token1]) => [
        token0.address,
        token1.address,
      ]),
    [chainId]
  ...
  const inputPanelHelperText = useMemo(() => {
    if (rate && trade) {
      const { numerator, denominator } = rate
        .subtract(trade.executionPrice)
        .divide(trade.executionPrice);
  ...
  return (
    <>
      <SwapLayoutCard>
        ...
        <div className="flex flex-col gap-3">
          <SwapAssetPanel
            ...
            header={(props) => (
              <SwapAssetPanel.Header
                ...
                walletCLass="flex items-center my-2 justify-between"
                {...props}
                isItemShow={true}
            ...
            walletToggle={(props) => (
              <SwapAssetPanel.Switch
                ...
                onChange={() =>
                  dispatch(setFromBentoBalance(!fromBentoBalance))
                }
              />
            )}
            selected={true}
            ...
            value={
              (typedField === Field.INPUT
                ? typedValue
                : parsedAmounts?.inputAmount?.toSignificant(6)) || ""
            }
            onChange={(value) => onUserInput(Field.INPUT, value || "")}
            onSelect={(inputCurrency) =>
              onCurrencySelection(Field.INPUT, inputCurrency)
            }
            currencies={inputTokenList}
          />
          <div className="grid grid-cols-03 gap-3 maxMd:flex maxMd:justify-around maxMd:mt-5">
            ...
            <div className="flex items-center justify-center maxMd:absolute maxMd:z-10  maxMd:-mt-24 z-[0]">
              <div
                role="button"
                className="p-3 rounded-full shadow-darkShadow bg-Gray-900 mt-1.5"
              >
                <ButtonSwap
                  className="p-3 rounded-full shadow-btnShadow bg-primary"
          ...
          <SwapAssetPanel
            ...
            value={
              (typedField === Field.OUTPUT
                ? typedValue
                : parsedAmounts?.outputAmount?.toSignificant(6)) || ""
            }
            onChange={(value) => onUserInput(Field.OUTPUT, value || "")}
            onSelect={(outputCurrency) =>
              onCurrencySelection(Field.OUTPUT, outputCurrency)
            }
            currencies={outputTokenList}
            priceImpact={inputPanelHelperText}
            priceImpactCss={
              inputPanelHelperText?.greaterThan(ZERO_PERCENT)
                ? "text-green"
                : "text-red"
            }
          />
      ...
      <Typography
        variant="xs"
        className="px-10 my-3 italic text-center text-white/60"
      >
        {i18n._(
      ...
      <Typography
        variant="xxs"
        className="px-10 mt-5 mb-10 italic text-center text-white/60"
      >
